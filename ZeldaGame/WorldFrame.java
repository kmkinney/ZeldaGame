...
public class WorldFrame<T> extends JFrame
{
    ...
    public WorldFrame(World<T> world)
    {
        ...
        ImageIcon appIcon = new ImageIcon("Triforce.png");
        ...
        KeyboardFocusManager.getCurrentKeyboardFocusManager().addKeyEventDispatcher(new
           KeyEventDispatcher() 
           {
               public boolean dispatchKeyEvent(KeyEvent event)
               {
                   ...
                   int r = text.indexOf(RELEASED);
                   if (n < 0 && r<0) return false; //typed
                   ...
                   //released
	           		text = text.substring(0, r)  + text.substring(r + RELEASED.length());
	           		boolean consumed = getWorld().keyReleased(text);
	           		if (consumed) repaint();
	           		return consumed;
